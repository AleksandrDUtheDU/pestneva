...
const FormWrapp = styled.form`
    ...
    @media ${props => props.theme.media.phone} {
        grid-template: 45px 45px 190px / 1fr;
    }
`
...
function Form() {
    ...
    async function postData(data) {
        ...
        const formData = new FormData(); // здесь мы преобразуем данные в формДату (так что бы их принял smart.php )
    ...
    return (
        <FormWrapp noValidate onSubmit={handleSubmit(onSubmit)}>
            ...
            <WrappInput>
                ...
                {errors?.email?.type === "pattern" && <ErrorMessage>Это поле должно содержать E-Mail в формате example@site.com</ErrorMessage>}
                <Label htmlFor="email" >Ваша почта</Label>
            </WrappInput>
            <WrappTextarea>
                <Textarea name="text" type="text" id="email" placeholder="Ваше сообщение..."
                    {...register("text", {
                        required: true,
                        maxLength: 200
                    })}
            ...
            <TriggersBox>
                ...
                <PoliticTextBox>
                    <PoliticInput name="checkbox" id="checkbox" type="checkbox"
                        {...register("checkbox", {
                            required: true,
                            checked: true,
                            maxLength: 200
